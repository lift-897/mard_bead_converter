...
> ⚠️ **下面的图片暂时显示不出来是正常的 —— 因为还没有截图。照下面的步骤 2 分钟搞定：**

### 🖼️ 手把手加截图教程

| 第几步 | 在哪里操作 | 具体做什么 |
|--------|-----------|------------|
| **第1步** | 浏览器打开工具页面 | 按 `Win + Shift + S`（Windows自带截图），框选界面，截图自动存到剪贴板 |
| **第2步** | 打开电脑画图软件 | 按 `Win`键→ 输入`画图`→ 回车。`Ctrl+V` 粘贴截图，`Ctrl+S` 保存 |
| **第3步** | 保存截图到仓库 | 保存位置选 `此电脑 → 桌面 → mard_bead_converter → screenshots`；文件名填 `converter.png`；点保存 |
| **第4步** | 回来改这行字 | 找到下面 `<!--` 和 `-->` 那两行，把 `<!--` 和 `-->` 删掉就行 |
| **第5步** | 推上GitHub | 在 VS Code 左侧点第三个图标（源代码管理），输入消息点提交，点同步 |

> 💡 用手机拍的实物照片也一样的道理，传到电脑上后放到 `screenshots` 文件夹就行

---

### 图案转换模式截图
<!-- 👇 放好截图后，删掉下面这行开头的 <!-- 和 末尾的 -->
